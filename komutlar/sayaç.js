...
exports.run = async (client, message, args) => {
  ...
  if (!mentionedChannel && args[0] !== "sıfırla") return message.channel.send(s1);
        ...
        if(args[0] === "sıfırla") {
                ...
                const embed = new Discord.RichEmbed()
                        .setDescription(`<a:tik:676049276689711138>Sayaç başarıyla sıfırlandı!`)
                        .setColor("RANDOM")
                        .setTimestamp()
                message.channel.send({embed})
                return
  ...
  profil[message.guild.id].kanal = mentionedChannel.id
        ...
        const embed = new Discord.RichEmbed()
                .setDescription(`<a:tik:676049276689711138> Sayaç başarıyla \`${args[0]}\` olarak ayarlandı! Sayaç kanalı ${mentionedChannel} olarak ayarlandı`)
                .setColor("RANDOM")
                .setTimestamp()
        message.channel.send({embed})
}
