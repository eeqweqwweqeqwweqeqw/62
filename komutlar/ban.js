...
exports.run = async (bot, message, args) => {
    if (!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send("<a:hayr:676049254388858880> Bu komudu kullanabilmen için `Üyeleri Yasakla` yetkisine sahip olmalısın.");
    ...
    message.channel.send(`\`${user.tag}\` adlı kişi sunucudan yasaklanacak? Kabul ediyorsanız **evet / e** etmiyorsanız **hayır / h** yazınız.`)
        let uwu = false;
            while (!uwu) {
                ...
                message.channel.send(`<a:tik:676049276689711138> **${user.tag}** adlı kullanıcı **${message.author.tag}** adlı yetkili tarafından **${reason}** sebebiyle sunucudan yasaklandı.`)
